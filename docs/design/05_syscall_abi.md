...
- 早期实现 `clock_gettime/gettimeofday/getpid`，支持 MONOTONIC/RAW/BOOTTIME 并返回 timebase 时间。
...
- 早期实现 `ioctl(TIOCGWINSZ/TIOCSWINSZ/TIOCGPGRP/TIOCSPGRP/TCGETS/TCSETS*)`，为终端提供窗口大小与最小 termios 占位。
