...
- 自研测例通过 `scripts/test_oscomp.sh` 统一驱动，读取 `tests/self/` 的用例列表，日志输出到 `build/selftest/`，需要时通过 `EXPECT_INIT=1` 检查 `/init` execve banner；ext4 用例通过 `EXPECT_EXT4=1` 检查 `vfs: mounted ext4 rootfs` 日志；ext4-init 用例通过 host 侧 VFS 读取验证 `/init` ELF 头与目录遍历。
